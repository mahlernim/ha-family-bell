...
class HaFamilyBellPanel extends HTMLElement {
  ...
  async saveSettings(button) { const root = this.shadowRoot; await this.saveWithFeedback(button, { type: "ha_family_bell/settings", changes: { tts_service: root.querySelector("#tts-service").value.trim(), language: root.querySelector("#language").value.trim(), intro_delay: Number(root.querySelector("#intro-delay").value), queue_hold_seconds: Number(root.querySelector("#queue-hold").value), intro_urls: root.querySelector("#intro-urls").value.split("\n").map((v) => v.trim()).filter(Boolean) } }); }
